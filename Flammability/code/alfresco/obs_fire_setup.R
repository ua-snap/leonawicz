...
if(file.exists(b.fid.name)) { b.fid <- brick(b.fid.name); print("b.fid read into memory") }
if(file.exists(result.name)) { result <- brick(result.name); print("result read into memory") }
if(file.exists(result2.name)) { result2 <- raster(result2.name); print("result2 read into memory") }
...
if(!exists("b.fid")){
	...
	if(substr(tolower(alf.domain),1,6)=="noatak") { b.fid <- mask(b.fid, shp); print("b.fid brick masked") }
	if(!file.exists(b.fid.name)){
		#writeRaster(b.fid, b.fid.name, datatype="FLT4S", overwrite=T)
		b <- brick(b.fid, values=FALSE)  
		b <- writeStart(b, filename=b.fid.name, format="GTiff", datatype="FLT4S", overwrite=TRUE)
		tr <- blockSize(b)
		for (i in 1:tr$n) {
...
if(!exists("result")){
	...
	if(!exists("result2")) result2 <- do.call("sum",c(result, na.rm=T))
	...
	if(!file.exists(result.name)){
		...
		b <- writeStart(b, filename=result.name, format="GTiff", datatype="FLT4S", overwrite=TRUE)
		tr <- blockSize(b)
		for (i in 1:tr$n) {
	...
	if(!file.exists(result2.name)) { writeRaster(result2, result2.name, datatype="FLT4S", overwrite=T); print("result2 file written") }
}
