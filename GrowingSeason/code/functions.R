# Cross-validation error data frame
get_cv_err <- function(.){
  k <- 200
  b <- .$CV[[1]]
  m <- .$GBM1[[1]]
...
make_TDD_dt <- function(d, extractBy, y, keep.y=FALSE, years=NULL, dem, tdd.pct=c("10"), ...){
  ...
  d <- d %>% purrr::map2(tdd.pct, ~mutate(.x, Threshold=paste0(.y, "pct"))) %>% bind_rows() %>% filter(Cell %in% cells) %>% group_by(Region, Year, Threshold) %>%
