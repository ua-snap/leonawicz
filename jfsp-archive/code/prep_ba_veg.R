...
d1 <- group_by(d[[1]], Tx, RCP, Model, Year, Replicate, FMO) %>% summarise(Val = sum(Val)) %>% ungroup %>%
  mutate(Vegetation = "Coniferous")
d2 <- group_by(d[[2]], Tx, RCP, Model, Year, Replicate, FMO) %>% summarise(Val = sum(Val)) %>% ungroup %>%
  mutate(Vegetation = "Deciduous")
d <- bind_rows(d1, d2) %>%  mutate(Vegetation = factor(Vegetation, levels = c("Coniferous", "Deciduous"))) %>%
  group_by(Tx, RCP, Model, Year, FMO, Vegetation) %>% summarise(Val = mean(Val)) %>% ungroup
saveRDS(d, "/workspace/UA/mfleonawicz/conif_decid_ba.rds")
