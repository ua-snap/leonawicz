library(alfresco)
library(dplyr)
file <- "data-raw/historical_fsv.rds"
x <- c("00", 50, 75, 98)
...
hcbr <- fmo_cb_reduction(d)
usethis::use_data(hcbr)

lev <- c("Control", "Tx: 50s/50i")
hcba_comp <- select(d, c(11, 7:10)) %>% filter(FMO %in% c("fmo00s00i", "fmo50s50i")) %>%
  mutate(
    FMO = factor(ifelse(FMO == "fmo00s00i", lev[1], lev[2]), levels = lev),
    FS = FS / 1000,
    CBA = CBA / 1000)
usethis::use_data(hcba_comp)
