...
denMod <- function(input, output, session, data, metric, axis_scale){
  ...
  output$plot1 <- renderPlot({ req(d(), axislab()); distPlot("density", list(rv_plots$x1, rv_plots$y1), axislab()) })
  output$plot2 <- renderPlot({ req(d(), axislab()); distPlot("histogram", list(rv_plots$x2, rv_plots$y2), axislab()) })
...
tsMod <- function(input, output, session, data, metric, axis_scale){
  ...
  output$plot1 <- renderPlot({ tsPlot("raw", list(rv_plots$x1, rv_plots$y1)) })
  output$plot2 <- renderPlot({ tsPlot("cumulative", list(rv_plots$x2, rv_plots$y2)) })
...
decMod <- function(input, output, session, data, metric, axis_scale){
  ...
  output$plot1 <- renderPlot({ decPlot("boxplot", list(rv_plots$xdec1, rv_plots$ydec1)) })
  output$plot2 <- renderPlot({ decPlot("barplot", list(rv_plots$xdec2, rv_plots$ydec2)) })
