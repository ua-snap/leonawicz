...
denMod <- function(input, output, session, data, metric, axis_scale){
  ...
  plot1 <- reactive({ distPlot("density", list(rv_plots$x1, rv_plots$y1), axislab()) })
  plot2 <- reactive({ distPlot("histogram", list(rv_plots$x2, rv_plots$y2), axislab()) })
  output$plot1 <- renderPlot({ req(d(), axislab()); plot1() })
  output$plot2 <- renderPlot({ req(d(), axislab()); plot2() })
...
tsMod <- function(input, output, session, data, metric, axis_scale){
  ...
  plot1 <- reactive({ req(d()); tsPlot("raw", list(rv_plots$x1, rv_plots$y1)) })
  plot2 <- reactive({ req(d()); tsPlot("cumulative", list(rv_plots$x2, rv_plots$y2)) })
...
decMod <- function(input, output, session, data, metric, axis_scale){
  ...
  output$plot1 <- renderPlot({ req(d()); plot1() })
  output$plot2 <- renderPlot({ req(d()); plot2() })
  ...
  output$statBoxes1 <- renderUI({
    ...
    y <- list(
      mean=valueBox(val[[1]], text[[1]], 
                    icon=apputils::icon(list(src=statIcon("mean"), width="90px"), lib="local"), color=clrs[1], width=NULL),
      min=valueBox(val[[2]], text[[2]], icon=apputils::icon(list(src=statIcon("min"), width="90px"), lib="local"), color=clrs[2], width=NULL),
      max=valueBox(val[[3]], text[[3]], icon=apputils::icon(list(src=statIcon("max"), width="90px"), lib="local"), color=clrs[3], width=NULL),
      med=valueBox(val[[4]], text[[4]], icon=apputils::icon(list(src=statIcon("median"), width="90px"), lib="local"), color=clrs[4], width=NULL),
      iqr=valueBox(val[[5]], text[[5]], icon=apputils::icon(list(src=statIcon("iqr"), width="90px"), lib="local"), color=clrs[5], width=NULL),
      sd=valueBox(val[[6]], text[[6]], icon=apputils::icon(list(src=statIcon("sd"), width="90px"), lib="local"), color=clrs[6], width=NULL)
    )
  ...
  output$statBoxes2 <- renderUI({
    ...
    if(tot < 0){
      src.totals <- c(statIcon("dec"), statIcon("pctdec"))
    } else {
      src.totals <- c(statIcon("inc"), statIcon("pctinc"))
    }
    ...
    text <- map2(statlab, rep(150, 6), ~pTextSize(.x, .y, margin=0))
    y <- list(
      mn=valueBox(val[[1]], text[[1]], icon=apputils::icon(list(src=statIcon("min"), width="90px"), lib="local"), color=clrs[1], width=NULL),
      mx=valueBox(val[[2]], text[[2]], icon=apputils::icon(list(src=statIcon("max"), width="90px"), lib="local"), color=clrs[2], width=NULL),
