...
shinyServer(function(input, output, session) {
  ...
  mapSelect <- reactive({
    ...
    x <- leaflet() %>% addTiles() %>% setView(lon, lat, 4) %>%
      addMapPane(name = "not_selected", zIndex = 420) %>% 
      addMapPane(name = "selected", zIndex = 430) %>%
      addMapPane(name = "simregion", zIndex = 410) %>%
      addPolygons(data=flam, stroke=TRUE, fillOpacity=0.2, weight=1, color="red", group="Simulation region", 
                  options = leafletOptions(pane = "simregion")) %>%
      addLayersControl(overlayGroups = c("Simulation region"))
    ...
    if(!ak){
      for(i in seq_along(z)){
        x <- x %>% addPolygons(data=subset(rv$shp, REGION==z[i]), stroke=TRUE, fillOpacity=0, weight=1,
          ...
          labelOptions = labelOptions(direction = "left", style = list(
                                      "font-size" = "14px", "font-style" = "bold", 
                                      "border-color" = "rgba(0, 0, 0, 1)")),
          highlightOptions=highlightOptions(opacity=1, weight=2, fillOpacity=0, 
            bringToFront=FALSE, sendToBack=FALSE), options = leafletOptions(pane = "not_selected"))
